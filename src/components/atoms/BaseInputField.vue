<template>
  ...
  <p v-show="isError()" class="text-status-error text-body-xs">
    {{ errorField.value ?? props.error }}
  </p>
</template>
...
const rule = ref<string | string[]>(props.rules as string | string[])
onMounted(() => {
  if (props.rules === '') {
...
let fieldName = computed(() => {
  return props.fieldName !== '' ? props.fieldName : (attrs.placeholder as string)
})
